...
Device_page::Device_page(QWidget *parent):  QMainWindow(parent)
{
    ...
    */
    ...
    Device_Name->setStyleSheet("QLabel{border: 1px solid #111; background-color: white};"); //rgb(220, 220, 220)}");
    ...
    Protocol_Name->setStyleSheet("QLabel{border: 1px solid #111; background-color: rgb(220, 220, 220)}");
...
void Device_page::readsettings()
{
    ...
    //qDebug() << "dir_SysTranslate: " << dir_SysTranslate;
...
void Device_page::slot_RequestCloseBlock(QMap<QString, QString> *map)
{
    ...
    if(map->keys().contains(barcode_name) && flag_Compatibility && RunDev->isEnabled())
    {
       ...
       if(!text.isEmpty())
       {
           ...
           Message->setIcon(QMessageBox::Question);
           text = tr("Do You want to run a protocol: ") + barcode + "?";
...
void Device_page::slot_StatusProtocol(QString status)
{
    ...
    case 1:     //Status_LoadProtocol->setPixmap(QPixmap(":/images/ok_pro.png"));
                ...
                Protocol_Name->setStyleSheet("QLabel{border: 1px solid #111; background-color: rgb(128, 255, 128)}");
                Device_Name->setStyleSheet("QLabel{border: 1px solid #111; background-color: rgb(128, 255, 128)}");
...
void Device_page::slot_RefreshMeasure(QMap<QString, QString> *map)
{
    ...
    //qDebug() << "slot_RefreshMeasure: " << T_current << fn << state;
    ...
    case sUnknown:      if(state > sNothing && state <= sWarming_up)
                        {
                            ...
                            Message->setText(tr("Attention! Program is running in the device!"));
                            ...
                            {/*
                                ...
                                fn = qApp->applicationDirPath() + "/device/run.rt";
...
//-----------------------------------------------------------------------------
bool Device_page::Create_MeasureProtocol(QString text)
{
    int i;
    bool res = false;
    bool ok;
    ...
    bool sts = QFile::remove(dir_device + "/measurements.rt");

    if(text.isEmpty())  file_out.setFileName(dir_device + "/measurements.rt");
